...
function createStoredProcedures() {
  ...
  sql = "CREATE PROCEDURE IF NOT EXISTS `getUserRole`(IN account_number INT)\n" +
  "BEGIN\n" +
  "	SELECT ut.user_type\n" +
  ...
  con.query(sql, function (err, results, fields) {
    if (err) {
      ...
      console.log("database.js: SP getUserRole created if it didn't exist");
    }
  });

  sql = "CREATE PROCEDURE IF NOT EXISTS `setUserRole`(IN account_number INT, IN user_type_id TINYINT)\n" +
  "BEGIN\n" +
  "	UPDATE `user` AS u\n" +
  "		INNER JOIN `account_number` AS an ON an.user_id = u.user_id\n" +
  ...
  con.query(sql, function (err, results, fields) {
    if (err) {
      ...
      console.log("database.js: SP setUserRole created if it didn't exist");
    }
  });
