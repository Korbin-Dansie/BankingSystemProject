...
function step1Manage(obj, res){
  ...
  dbCon.query(sql, [obj.accountNumber], function(err,rows){
    ...
    if(rows[0][0].result == 0){
      res.render('manage', { message: 'Account number: ' + obj.accountNumber + " does not exist."});
    }
    else{
      step2Manage(obj, res);
    }
...
function step3Manage(obj, res){
  ...
  dbCon.query(sql, [obj.accountNumber], function(err, rows){
    ...
    res.render('manageResults', {accountNumber: obj.accountNumber, lastName: obj.lastName, firstName: obj.firstName, userRole: obj.userRole});
  });
}
...
function step1ManagePassword(obj, res){
  ...
  dbCon.query(sql, [obj.accountNumber, obj.administration], function(err,rows){
    if(err){
      throw err;
    }
    res.redirect("/");
